...
$stmt = $db->prepare("SELECT * FROM " . TABLE_PREFIX . "faq WHERE faq_id = ?");
$stmt->execute([$faqId]);
$faq = $stmt->fetch();

if (!$faq) {
    redirect('/admin/manage-faq.php');
    exit;
}
    ...
    <div class="content-area">
        ...
        <form method="POST" action="" id="editFaqForm">
            <div class="form-group">
                ...
                <input type="text" id="question" name="question" value="<?= h($faq['question']) ?>" required>
            </div>
            
            <div class="form-group">
                <label for="excerpt">Preview/Excerpt:</label>
                <textarea id="excerpt" name="excerpt" rows="2"><?= h($faq['excerpt'] ?? '') ?></textarea>
            ...
            <div class="form-group">
                ...
                <textarea id="answer" name="answer" rows="15" required><?= h($faq['answer']) ?></textarea>
            </div>
            ...
            <div class="form-group">
                <label class="checkbox-label">
                    <input type="checkbox" id="is_active" name="is_active" value="1" <?= $faq['is_active'] ? 'checked' : '' ?>>
                    Active
                </label>
            </div>
            ...
            <div class="form-group">
                ...
                <a href="<?= baseUrl('/admin/manage-faq.php') ?>" class="btn btn-secondary">Cancel</a>
            </div>
        </form>
